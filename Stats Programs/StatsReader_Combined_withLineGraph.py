...
def countUniqueInteractions(data):
    ...
    data_local = data_local.sort_values(by=['UserID', 'datetime'])
    ...
    for index, row in data_local.iterrows():
        if current_user is None or current_user != row['UserID'] or \
...
def plotInteractionsPerDay(data, ax):
    ...
    data_for_daily = data_for_daily.sort_values(by=['UserID', 'datetime'])
    ...
    for index, row in data_for_daily.iterrows():
        # Check for new interaction (10-minute window)
        if current_user is None or current_user != row['UserID'] or \
                row['datetime'] > current_interaction_start + timedelta(minutes=10):
            current_user = row['UserID']
            current_interaction_start = row['datetime']
...
while True:
    ...
    # Calculate per-user interaction counts
    interactions_count = {}
    current_user = None
    current_interaction_start = None
