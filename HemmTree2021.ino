  ...
  if (acceptingInput) {
    ...
    if (messageTemp.substring(0, 5) == "OTHER") { //modes that have parts generated/animated locally, but end static
      ...
      if (messageTemp.substring(5) == "fred") {
        ...
        //t
        for (int i = 0; i < stripLength; i++) {
          strip.setPixelColor(i, strip.Color(tempStorage[i][0], tempStorage[i][1], tempStorage[i][2])); //DO NOT use stripUpdate()--it will incorrectly swap colors since we are using getpixelcolor
        }
        strip.show();
        delay(dashDelay);
    ...
    if (messageTemp.substring(0, 5) == "SHORT") { //for modes that do an animation, then return to previous (so they don't interrupt dynam
      ...
      if (messageTemp.substring(5) == "bluey") {
        ...
        for (int k = 0; k < sizeof(sequence) / sizeof(sequence[0]); k++) {
          ...
          for (int i = startingPoint; i < startingPoint + segmentLength; i++) {
            stripUpdate(i, 40, 10, 202);
          }
        ...
        //Serial.println("done with Bluey");
      }
